...
const WEIGHTS = {
  POSTGRES_DEPENDENCY: 1.0,
};

/**
 * Capability detector for the PostgreSQL database.
 * Gathers evidence based on package manifest declarations.
 */
export class PostgresDetector implements Detector {
  ...
  async detect(context: EvidenceContext): Promise<{
    ...
    const pgPackages = ['postgres', 'pg', 'pg-native'];
    for (const pkg of pgPackages) {
      if (context.node.hasPackage(pkg)) {
        evidence.push({
          ...
          message: `Found PostgreSQL driver "${pkg}" dependency in package.json`,
          weight: WEIGHTS.POSTGRES_DEPENDENCY,
        });
