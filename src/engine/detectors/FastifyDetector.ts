...
const WEIGHTS = {
  FASTIFY_DEPENDENCY: 1.0,
};

/**
 * Capability detector for the Fastify framework.
 * Gathers evidence based on package manifest declarations.
 */
export class FastifyDetector implements Detector {
  readonly id = 'fastify';
  readonly label = 'Fastify';
  readonly category = CapabilityCategory.Framework;
  readonly role = 'server-framework';
  readonly threshold = 0.3;

  /**
   * Scans the repository context and gathers items of evidence of Fastify.
   */
  async detect(context: EvidenceContext): Promise<{
    ...
    if (context.node.hasPackage('fastify')) {
      evidence.push({
        type: EvidenceType.Dependency,
        source: { type: EvidenceSourceType.Manifest, name: 'package.json' },
        message: 'Found "fastify" dependency in package.json',
        weight: WEIGHTS.FASTIFY_DEPENDENCY,
      });

      const v = context.node.getPackageVersion('fastify');
      if (v) version = v;
    }
